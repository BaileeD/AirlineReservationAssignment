/*This class, SimpleJet, extends airline. It inherits airlines methods and instance
 * variables. 
 * @author Bailee Devey*/
public class SimpleJet extends Airline{
	
	/*This is a default constructor which sets the name of the airline to "Simple Jet".*/
	public SimpleJet() {
		this.setAirlineName("Simple Jet");
	}
	 ...
	 *on the left or the right of that seat. If there is no seats matching that criteria then it searches for a 
	 *window seat in economy and reserves it, along with the seat on the right. 
	 *@param aFlight, aSeat
	 *@return s*/
	@Override
	Seat reserveFirstClass(FlightInformation aFlight, SEATTYPE aSeat) {
		...
		s3 = aFlight.getaSeatMap().queryAvailableEconomySeat(SEATTYPE.WINDOW);
	
		if (s2 != null && aFlight.getaSeatMap().getLeft(s2).getReserved() != true){
			s2.setReserved(true);
			aFlight.getaSeatMap().getLeft(s2).setReserved(true);
			return s2;
		} else if (s2 != null && aFlight.getaSeatMap().getRight(s2).getReserved() != true){
			s2.setReserved(true);
			aFlight.getaSeatMap().getRight(s2).setReserved(true);
			return s2;
		} else if (s3 != null && aFlight.getaSeatMap().getLeft(s3).getReserved() != true){
			s3.setReserved(true);
			aFlight.getaSeatMap().getLeft(s3).setReserved(true);
			return s3;
		} else if (s3 != null && aFlight.getaSeatMap().getRight(s3).getReserved() != true){
			s3.setReserved(true);
			aFlight.getaSeatMap().getRight(s3).setReserved(true);
			return s3;
		} else {
			return null;
		}
